import { borrowTokens, canBurrow } from "./db";
...
export async function borrow(amount: number): Promise<void> {
  ...
  const FAUCET_ID2 = process.env.PUBLIC_NEXT_FAUCET_ID2 || "0xe62152de7f96cb202fb80fa1d0fe83";

  const data =  await canBurrow(USER_ID);
  if (!data.canBurrow) {
    console.error(
  ...
  if (!faucet2) {
    console.error("Failed to fetch Faucet's 2 account. Please check the account ID.");
    return;
  }
  ...
  // ── Create unauthenticated note transfer chain ─────────────────────────────────────────────
    ...
    const assets = new NoteAssets([new FungibleAsset(faucet2.id(), BigInt(amount))]);
  ...
  // ── Create unauthenticated note transfer chain ─────────────────────────────────────────────
    ...
    const assets1 = new NoteAssets([new FungibleAsset(faucet.id(), BigInt(Math.floor(amount/2)))]);
  ...
  await borrowTokens(USER_ID, amount);
