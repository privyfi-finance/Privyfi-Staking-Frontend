import { canWithdrawlend, lendTokens, withdrawlend } from "./db";
...
export async function withdraw_lend(amount: number): Promise<void> {
  ...
  const data = await canWithdrawlend(USER_ID, amount);
  if (!data.canWithdraw) {
    console.error(
  ...
  await withdrawlend(user.id().toString(), amount);
