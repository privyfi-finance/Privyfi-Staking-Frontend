...
export default function WithdrawalsPage() {
  ...
  const handleWithdraw = async (id: string, amount: number): Promise<void> => {
    ...
    if (result === true) {
      ...
      toast.error("Withdrawal failed. Please check staked amount.");
    }
    setIsLoading(false);
  };
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex bg-gray-100 rounded-lg p-1 mb-8 max-w-xs mx-auto">
        <button
          ...
          className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${withdrawalTab === "request" ? "bg-white text-gray-900 shadow-sm" : "text-gray-600 hover:text-gray-900"
            }`}
        >
          Request
        </button>
        <button
          ...
          className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${withdrawalTab === "claim" ? "bg-white text-gray-900 shadow-sm" : "text-gray-600 hover:text-gray-900"
            }`}
        >
          Claim
        </button>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-8">
        <div className="p-6">
          <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div
                    ...
                    className={`p-4 border-2 rounded-lg cursor-pointer transition-colors ${withdrawalMethod === "privyfi"
                  ...
                  <div
                    ...
                    className={`p-4 border-2 rounded-lg cursor-pointer transition-colors ${withdrawalMethod === "dexs"
                      ? "border-orange-500 bg-orange-50"
                      : "border-gray-200 hover:border-gray-300"
                      }`}
                  >
                ...
                <button className="w-full bg-orange-500 hover:bg-orange-600 text-white py-3 rounded-lg font-medium" onClick={() => handleWithdraw(accountId ?? '', Number(stethAmount))}>
                  {isLoading ? <ClipLoader color="#FFFFFF" /> : "Request withdrawal"}
                </button>
              </div>
            )}
            ...
            {withdrawalTab === "claim" && (
              <div className="space-y-4">
                <button className="w-full bg-orange-500 hover:bg-orange-600 text-white py-3 rounded-lg font-medium" onClick={() => handleClaim(accountId ?? '')}>
                  {isLoading ? <ClipLoader color="#FFFFFF" /> : "Request Claim"}
                </button>
              </div>
            )}
